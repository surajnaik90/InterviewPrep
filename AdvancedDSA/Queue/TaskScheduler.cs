...
specific order to avoid deadlock. In every clock cycle, the CPU can either perform a task or move it to 
the back of the queue. You are given the current state of the scheduler queue in array A and 
the required order of the tasks in array B.
...
According to the order array B task 1 has to be performed first, so the CPU will move tasks 2 and 3 to the 
end of the queue (in 2 clock cycles).
...
public static class TaskScheduler
{
    public static int solve(List<int> A, List<int> B)
    {
        ...
        while (state.Count > 0) {
            ...
            if(state.Peek() != order.Peek()) {
                while (state.Peek() != order.Peek()) {
                    totalCycles++;
