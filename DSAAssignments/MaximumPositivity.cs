...
public static class MaximumPositivity
{
    public static List<int> Operation1(List<int> A)
    {
        List<int> output = new List<int>();

        int maxCount = 0, minIndex = int.MaxValue, currentCount=0, currentIndex=-1, i;
        ...
        for (i = 0; i < A.Count; i++)
        {
            ...
            else if(A[i] < 0 && currentCount != 0){

                isPositive = false;
                if(currentCount == maxCount) {
                    minIndex = (i - maxCount )< minIndex ? (i - maxCount) : minIndex;
                    currentCount = 0;
                    continue;
                }
                if (currentCount > maxCount){
        ...
        if (isPositive) { minIndex = 0; maxCount = A.Count; }

        for (int j = minIndex; j < minIndex + maxCount; j++) {
            output.Add(A[j]);
        }
