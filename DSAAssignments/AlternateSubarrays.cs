...
public static class AlternateSubarrays
{
    public static List<int> Operation1(List<int> A, int B)
    {
        List<int> output = new List<int>();
        int N = A.Count, subArrSize = 2 * B + 1, s, e;
        bool isAlternate;

        for (s = 0; s < N; s++)
        {
            int endIndex = s + subArrSize - 1;

            if(s+subArrSize<=N)
            {
                isAlternate = true; int prev = A[s];
                for (e = s+1; e <= endIndex; e++)
                {
                    if(A[e] == prev) { 
